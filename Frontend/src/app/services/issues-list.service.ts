...
export class IssuesListService {
  ...
  private apiURL = `${BASE_URL}${ISSUE_PAGE_ENDPOINT}?projectId=${PROJECT_ID}`
  constructor(private http:HttpClient) { }

  getData(state?: string, labels?: string[]):Observable<any>{
    let newState: string ="";
    ...
    return this.http.get<any[]>(`${this.apiURL}${newState}${labelList}`, httpOptions);
  }
}